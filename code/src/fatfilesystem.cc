...
int FatFileSystem::Open(const std::string &filename, const int &mode) {
...
void FatFileSystem::Read(const int &file_number, std::string &data, const int &read_size) {
  int size = read_size;
  int index = BinarySearchInBuffer(file_number);
  if (index == -1) {
...
void FatFileSystem::Write(const int &file_number, const std::string &data, const int &write_size) {
  int index = BinarySearchInBuffer(file_number);
  if (index == -1) {
    std::cout << "FILE IS NOT OPEN" << std::endl;
    exit(-1);
...
void FatFileSystem::Close(const int &file_number) {
  int index = BinarySearchInBuffer(file_number);
  if (index == -1)
    return ;
...
int FatFileSystem::BinarySearchInBuffer(const int &file_number) {
...
bool FatFileSystem::CmpByFileNumber(const FileStatus &fs1, const FileStatus &fs2) {
  return fs1.file_number < fs2.file_number;
}


int FatFileSystem::AssignFreeBlocks() {
...
void FatFileSystem::FreeBlock(const int &lba) {
  fat[lba] = lba;
