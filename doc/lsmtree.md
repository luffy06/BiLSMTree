...
$f_{k}$，自顶向下第$k$个SSTable文件的访问频率；$L[k]$，该文件的层数。
$$
E[Extra\_IO]=\sum_k f_{k}\times (L[k] + 1)
\\
N[0]=4,N[1] = 10, N[i]=N[i - 1]\times r\quad (i \ge 2,2\le r \le 10)
\\
S[0]=0,S[i] = S[i-1] + N[i - 1]
$$
**分配层数：**

若第k个文件满足，$S[i-1]\le k\le S[i]$，则文件属于第i-1层。
